...
logo_img = imread('images/pk.png');
logo_img2 = imread('images/pk2.png');
% Generate logo points (they are just the outer corners of the image)
[logoy, logox, ~] = size(logo_img);
[logoy2, logox2, ~] = size(logo_img2);
%logo_pts = [0 0; logox 0; logox logoy; 0 logoy];
logo_pts = [0 logoy; logox logoy; logox 0; 0 0];
logo_pts2 = [0 logoy2; logox2 logoy2; logox2 0; 0 0];
...
for i=1:num_frames
    ...
    %logo left
    video_pts = zeros(4,2);
    video_pts(1,:) = proj_pts(4,:);
    video_pts(2,:) = proj_pts(1,:);
    video_pts(3,:) = proj_pts(5,:);
    video_pts(4,:) = proj_pts(8,:);
    [ interior_pts ] = calculate_interior_pts(size(generated_imgs{i}),video_pts);
    
    % Warp the interior_pts to coordinates in the logo image
    warped_logo_pts = warp_pts(video_pts,logo_pts2,interior_pts);
    
    % Copy the RGB values from the logo_img to the video frame
    generated_imgs{i} = inverse_warping(generated_imgs{i},logo_img2,interior_pts,warped_logo_pts);
    
    %logo up
    video_pts(1,:) = proj_pts(3,:);
    video_pts(2,:) = proj_pts(4,:);
    video_pts(3,:) = proj_pts(8,:);
    video_pts(4,:) = proj_pts(7,:);
    [ interior_pts ] = calculate_interior_pts(size(generated_imgs{i}),video_pts);
    
    % Warp the interior_pts to coordinates in the logo image
    warped_logo_pts = warp_pts(video_pts,logo_pts,interior_pts);
    
    % Copy the RGB values from the logo_img to the video frame
    generated_imgs{i} = inverse_warping(generated_imgs{i},logo_img,interior_pts,warped_logo_pts);
